...
class Anime {
  ...
  setAnimationsProgress (insTime) {
    ...
    for (let i = 0; i < animsLen; i++) {
      ...
      // 获取在当前 insTime 中对应的值
      const elapsed = minMaxValue(insTime - tween.start, 0, tween.duration) / tween.duration
      const eased = this.easing(elapsed, this.elasticity)
      const value = eased * (tween.to - tween.from) + tween.from

      // target 赋值
      this.target[anim.key] = value
    }
